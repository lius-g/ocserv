...
Download_ocserv(){
	...
	tar -xJf ocserv-0.11.12.tar.xz && cd ocserv-0.11.12
	./configure
	make
	make install
	cd .. && cd ..
	...
	if [[ -e ${file} ]]; then
		mkdir "${conf_file}"
		wget --no-check-certificate -N -P "${conf_file}" "https://raw.githubusercontent.com/lius-g/ocserv/master/ocserv.conf"
...
Service_ocserv(){
	if ! wget --no-check-certificate https://raw.githubusercontent.com/lius-g/ocserv/master/ocserv_debian -O /etc/init.d/ocserv; then
...
Set_passwd(){
	...
	read -e -p "(默认: 12345678):" userpass
	[[ -z "${userpass}" ]] && userpass="12345678"
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/lius-g/ocserv/master/ocserv.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/ocserv" ]]; then
		rm -rf /etc/init.d/ocserv
		Service_ocserv
	fi
	wget -N --no-check-certificate "https://raw.githubusercontent.com/lius-g/ocserv/master/ocserv.sh" && chmod +x ocserv.sh
