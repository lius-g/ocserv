...
Download_ocserv(){
	...
	if [[ -e ${file} ]]; then
		mkdir "${conf_file}"
		wget --no-check-certificate -N -P "${conf_file}" "https://github.com/lius-g/ocserv/blob/master/ocserv.conf"
...
Service_ocserv(){
	if ! wget --no-check-certificate hhttps://github.com/lius-g/ocserv/blob/master/ocserv_debian -O /etc/init.d/ocserv; then
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://github.com/lius-g/ocserv/edit/master/ocserv.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/ocserv" ]]; then
		rm -rf /etc/init.d/ocserv
		Service_ocserv
	fi
	wget -N --no-check-certificate "https://github.com/lius-g/ocserv/edit/master/ocserv.sh" && chmod +x ocserv.sh
